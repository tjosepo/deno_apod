...
Deno.test("normal page, copyright", async () => {
  const data = await apod("2017-03-22");
  assertEquals(data, {
...
Deno.test("newer page, Reprocessing & copyright", async () => {
  const data = await apod("2017-02-08");
  assertEquals(data, {
...
Deno.test("older page, copyright", async () => {
  const data = await apod("2015-11-15");
  assertEquals(data, {
    ...
    hdurl:
      "https://apod.nasa.gov/apod/image/1511/leonidsmonuments_sabatini_2330.jpg",
    media_type: "image",
    service_version: "v1",
    title: "Leonids Over Monument Valley",
    url:
      "https://apod.nasa.gov/apod/image/1511/leonidsmonuments_sabatini_960.jpg",
  });
});
Deno.test("older page, copyright #2", async () => {
  const data = await apod("2013-03-11");
  assertEquals(data, {
...
Deno.test("older page, no copyright", async () => {
  const data = await apod("1998-06-19");
  assertEquals(data, {
    date: "1998-06-19",
    copyright: undefined,
    explanation:
...
Deno.test("older page, no copyright, #2", async () => {
  const data = await apod("2012-08-30");
  assertEquals(data, {
    date: "2012-08-30",
    copyright: undefined,
    explanation:
